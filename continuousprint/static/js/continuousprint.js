				...
				success:function(r){
					...
					if (r.queue.length > 0) {
						...
						for(var i = 0; i < r.queue.length; i++) {
                            ...
                            row = $("<div class='n"+i+"'style='padding: 10px;border-bottom: 1px solid #000;"+(i==0 ? "background: #c7c2fc;" : "")+"'><div class='queue-row-container'><div class='queue-inner-row-container'>"+ "<p class='file-name' > " + file.name + "</p></div><div>" + other + "<i style='cursor: pointer' class='fa fa-minus text-error' data-index='"+i+"'></i></div></div></div>");
